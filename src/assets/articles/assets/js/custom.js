...
// -------------------------------------------------------------------------- // 
    ...
    $(document).ready(function(){
        // Define functions to handle calculating the measurement scale
        function calcF(temp) {return Math.round((temp * 1.8) + 32)};
        function calcC(temp) {return Math.round((temp - 32) * .5556)};
        // Define function to get geolocation, then call the API and place the data into proper locations in DOM.
            ...
            if(navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    $.getJSON("https://fcc-weather-api.glitch.me/api/current?lon=" + lon + "&lat=" + lat, function(data) {
                        console.log(data);
                        $("#user-location").html(data.name);
                        $("#summary").html(data.weather[0].main);
                        $("#temp").html(data.main.temp + "&deg;C");
                        $("#icon").removeClass("freecodecamp-background");
                        $("#icon").attr("src", data.weather[0].icon);
                        $("#high").html(data.main.temp_max);
                        $("#low").html(data.main.temp_min);
                        $(".scale").html("C");
                        $("#scale-btn").html("Fahrenheit");
                        $("#humidity").html("Humidity: " + data.main.humidity + "%");
                    })
                });
            }
        }
        // Define function to handle switching the scale on the DOM
        function switchScale() {
            var scaleBtn = $("#scale-btn");
            var high = $("#high");
            var highVal = high.html();
            var low = $("#low");
            var lowVal = low.html();
            if(scaleBtn.html() === "Fahrenheit") {
                scaleBtn.html("Celcius");
                high.html(calcF(highVal));
                low.html(calcF(lowVal));
                $(".scale").html("F");
            } else {
                scaleBtn.html("Fahrenheit");
                high.html(calcC(highVal));
                low.html(calcC(lowVal));
                $(".scale").html("C");
            }
        };
        // Start the app
        getWeatherData();
        // Define what happens on click for the scale button
        $("#scale-btn").on("click", switchScale);
    });
    $(document).ready(function(){
